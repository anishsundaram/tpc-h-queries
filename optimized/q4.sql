...
CREATE INDEX idx_l_commitdate_l_receiptdate on lineitem(l_commitdate, l_receiptdate) where (l_commitdate < l_receiptdate);
CREATE INDEX idx_order on orders (O_ORDERDATE);
SELECT
    O_ORDERPRIORITY,
...
ORDER BY
    O_ORDERPRIORITY;
DROP INDEX idx_l_commitdate_l_receiptdate;
